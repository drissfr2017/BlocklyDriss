...
Ardublockly.bindActionFunctions = function() {
  ...
  /* Supprimer par Driss car j'ai commenté les bouton dans index.html
  ...
  Fin de la suppression par driss*/
...
Ardublockly.setCompilerLocationHtml = function(newEl) {
  ...
  if (compLocIp != null) {
    compLocIp.value = newEl.value || compLocIp.value ||
        'Indiqer l\'emplacement du programme arduino.exe';
...
Ardublockly.setSketchLocationHtml = function(newEl) {
  ...
  if (sketchLocIp != null) {
    sketchLocIp.value = newEl.value || sketchLocIp.value ||
        'Indiquer le répertoire de sauvegarde du code adruino';
...
Ardublockly.renderContent = function() {
  ...
  /*Enlever par Driss parceque le volet XML a été retiré */
